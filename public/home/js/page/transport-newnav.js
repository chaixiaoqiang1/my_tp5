...
$(function(){
	...
	//右侧TAB按钮控制打开、缩回
	$('.close_group .toolbar_down').click(function(){
		$(this).find('i').toggleClass('icon-down').toggleClass('icon-upward');
		$('.close_group .toolbar_list').slideToggle(200);
	});

	$('.close_group .toolbar_list li a').click(function(){
		$(this).parents('.close_group').find('.toolbar_down').find('i').toggleClass('icon-upward').toggleClass('icon-down');
		$('.close_group .toolbar_list').slideToggle(200);
	});
	...
	//关闭操作
	$('.tab_active').on('click',o);
	$('.tab_closeOther').on('click',r);
	$('.tab_closeAll').on('click',function(){
		$(".page_tabs_b .page_tabs_content a").not(":first").each(function() {

			$('.page_content[tag="' + $(this).attr("tag") + '"]').remove();
			$(this).remove();
		});

		$(".page_tabs_b .page_tabs_content a:first").each(function() {
			$('.page_content[tag="' + $(this).attr("tag") + '"]').addClass('show');
			$(this).addClass("active")
		});

		$(".page_tabs_content").css("margin-left", "0")
	});
});
...
function addTabBar(url,title,tag){
	...
	if(!isTag){
		...
		//架注入iframe
		var iframeDiv = '<div class="page_content show animated fadeInRight" tag="'+tag+'" style="height:'+Wh+'px"><div class="load_div" style="width: 100%;height: 100%;background: #ffffff;position:absolute;left: 0;top: 0;z-index: 9"><img src="img/page/loading.gif" style="position:absolute;left: 50%;top: 50%;margin-left: -90px;margin-top: -120px;" /></div><iframe scrolling="yes" frameborder="0" src="'+url+'" style="position:absolute;left: 0;top: 0;" onload="stateChangeFirefox(this)"></iframe></div>';
...
function o() {
	e($(".page_tabs_b .page_tabs_content a.active"))
}


function r() {
	$(".page_tabs_b .page_tabs_content a").not(":first").not(".active").each(function() {
		$('.page_content[tag="' + $(this).attr("tag") + '"]').remove();
		$(this).remove()
	});
	$(".page_tabs_content").css("margin-left", "0")
